...
class Subscription(sp.Contract):
    ...
    def isSubsReady(self,params):
        ...
        self.addHashIfNecessary(elements)
        self.data.times[elements].ready = sp.bool(False)
        sp.verify(sp.check_signature(params.subs_pub_key, params.userSignature,elements))
        sp.verify(params.subscriber != params.publisher)
        sp.verify(sp.now >= self.data.times[elements].nextValidTimeStamp)
        ...
        self.data.times[elements].ready = sp.bool(True)
    ...
    def cancelSubs(self,params):
        ...
        sp.verify(sp.check_signature(params.subs_pub_key, params.userSignature,elements))
        sp.verify(params.subscriber == sp.sender)
        self.data.times[elements].nextValidTimeStamp = sp.timestamp(-1)
    ...
    def executeSubs(self,params):   
        ...
        sp.verify(self.data.times[elements].ready == sp.bool(True))
        self.data.times[elements].nextValidTimeStamp = sp.now.add_seconds(params.periodSeconds)
...
if "templates" not in __name__:
    ...
    def test():
        ...
        #
        message_subs_alice = sp.pack(sp.record(s = alice.address, p = admin.address, tA = c2.address, a = 1, pS = 60, gP = 0, n = 1))
        sig_from_alice = sp.make_signature(secret_key = alice.secret_key,
        ...
        scenario += c1.executeSubs(params = sp.record(subscriber = alice.address, publisher = admin.address, tokenAddress = c2.address, tokenAmount = 1, periodSeconds = 60, gasPrice = 0,nonce = 1, userSignature = sig_from_alice, subs_pub_key = alice.public_key)).run(valid = True)
        ...
        scenario += c1.executeSubs(params = sp.record(subscriber = alice.address, publisher = admin.address, tokenAddress = c2.address, tokenAmount = 1, periodSeconds = 60, gasPrice = 0,nonce = 1, userSignature = sig_from_alice, subs_pub_key = alice.public_key)).run(valid = True,now = 60)
        ...
        scenario += c1.cancelSub(params = sp.record(subscriber = alice.address, publisher = admin.address, tokenAddress = c2.address, tokenAmount = 1, periodSeconds = 60, gasPrice = 0,nonce = 1, userSignature = sig_from_alice, subs_pub_key = alice.public_key)).run(valid = True,sender = alice)
        #
        ...
        scenario.h2("Execute Subscription - 2")
        scenario += c1.executeSubs(params = sp.record(subscriber = alice.address, publisher = admin.address, tokenAddress = c2.address, tokenAmount = 100, periodSeconds = 60, gasPrice = 5,nonce = 1, userSignature = sig_from_alice, subs_pub_key = alice.public_key)).run(valid = True,now = 60)
